...
if(isMasterThread){
  printf("chalo barrier shuru kare for %d\n", blockIdx.x);
}
 __syncthreads();
...
void gg_main_pipe_1_wrapper(CSRGraph& gg, int& LEVEL, PipeContextT<Worklist2>& pipe, dim3& blocks, dim3& threads)
{
  ...
  else
  {
    ...
    gg_main_pipe_1_gpu_gb<<<80, __tb_gg_main_pipe_1_gpu_gb>>>(gg,LEVEL,pipe,cl_LEVEL, enable_lb, gg_main_pipe_1_gpu_gb_barrier, global_sense, perSMsense, done, global_count, local_count, last_block, NUM_SM);
